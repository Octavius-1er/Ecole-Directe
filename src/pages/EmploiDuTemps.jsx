...
const DAYS=[{day:9,key:"9",label:"Lundi 9"},{day:10,key:"10",label:"Mardi 10"},{day:11,key:"11",label:"Mercredi 11"},{day:12,key:"12",label:"Jeudi 12"},{day:13,key:"13",label:"Vendredi 13"},{day:14,key:"14",label:"Samedi 14"}];
const MAT_COLORS={"Français":"#3b82f6","Mathématiques":"#8b5cf6","Anglais LV1":"#0ea5e9","Allemand LV2":"#f59e0b","Histoire-Géo":"#10b981","Sciences Vie & Terre":"#22c55e","Physique-Chimie":"#f97316","Technologie":"#6366f1","EPS":"#ef4444","Arts Plastiques":"#ec4899","Éducation Musicale":"#14b8a6","LCA Latin":"#a16207"};
...
export default function EmploiDuTemps(){
  ...
  function handleAdd(e){
    ...
    addEdtItem(classeId,activeDay,dayData.label,{heure:`${newCours.heure} - ${newCours.heureFin}`,matiere:newCours.matiere,prof:newCours.prof,salle:newCours.salle});
    setMsg("Cours ajouté ✓");
  ...
  return(
    <div className="edt-container">
      ...
      <div className="edt-days-nav">
        {DAYS.map(d=>(
          <button key={d.key} className={`edt-day-btn ${activeDay===d.key?"active":""}`} onClick={()=>setActiveDay(d.key)}>{d.label}</button>
        ))}
      </div>
      ...
      {canEdit&&showAdd&&classeId&&(
        <form className="add-note-form" onSubmit={handleAdd}>
          <input type="time" value={newCours.heure} onChange={e=>setNewCours(p=>({...p,heure:e.target.value}))} required title="Début"/>
          <input type="time" value={newCours.heureFin} onChange={e=>setNewCours(p=>({...p,heureFin:e.target.value}))} required title="Fin"/>
      ...
      {!classeId&&<div className="edt-empty">Sélectionnez une classe pour voir l'emploi du temps.</div>}
                ...
                {dayData.items.map((item,i)=>{
                  ...
                  return(
                    <div key={i} className="edt-slot" style={{borderLeftColor:color}}>
                      ...
                      {canEdit&&<button className="btn-del-sm" onClick={()=>removeEdtItem(classeId,activeDay,i)}>✕</button>}
                    </div>
                  );
                })}
              </div>
